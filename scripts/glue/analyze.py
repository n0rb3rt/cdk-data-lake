...
w = Window.partitionBy("district", "primary_type").orderBy("month")
...
window1 = DynamicFrame.fromDF(
    ...
    dataframe=datasource0.toDF() \
        ...
        .groupBy("district", "primary_type", "month") \
        .agg(count("id").alias("monthly_count")) \
        .sort("district", "primary_type", "month") \
        .withColumn("prev_month", lag("month").over(w)) \
        .withColumn("prev_count", lag("monthly_count").over(w)) \
        .withColumn("prev_month", expr("case when last_day(add_months(prev_month,1)) = last_day(month) then prev_count else null end")) \
        .withColumn("monthly_change", (col("monthly_count") - col("prev_count")) / col("monthly_count")) \
...
datasink2 = glueContext.write_dynamic_frame.from_options(
    ...
    connection_type="s3",
    connection_options={
        "path": f"{out_bucket}/crime_trends"
    },
    format="parquet",
